...
import (
	...
	pluginrpc "github.com/kildevaeld/projects/projects/plugin/rpc"
)
...
func (c *Client) Hook() (projects.Hook, error) {
...
func (c *Client) UI() (projects.UI, error) {
	client, err := c.packrpcClient()
	if err != nil {
		return nil, err
	}

	return &cmdUI{client.UI(), c}, nil
}
...
func (c *Client) packrpcClient() (*pluginrpc.Client, error) {
	addr, err := c.Start()
	if err != nil {
		return nil, err
	}
	...
	client, err := pluginrpc.NewClient(conn)
	if err != nil {
		conn.Close()
		return nil, err
	}
